...
import com.pencil.prescription.callbacks.Callback;
import com.pencil.prescription.callbacks.CropCallback;
import com.pencil.prescription.callbacks.CropRequest;
import com.pencil.prescription.callbacks.LoadCallback;
import com.pencil.prescription.callbacks.LoadRequest;
import com.pencil.prescription.callbacks.SaveCallback;
import com.pencil.prescription.callbacks.SaveRequest;
import com.pencil.prescription.entity.SimpleValueAnimator;
import com.pencil.prescription.entity.SimpleValueAnimatorListener;
import com.pencil.prescription.entity.ValueAnimatorV14;
import com.pencil.prescription.entity.ValueAnimatorV8;
...
@SuppressWarnings("unused") public class CropImageView extends ImageView {
  ...
  private Bitmap mSourceUri = null;
...
//    this.mOutputImageHeight = ss.outputImageHeight;
  ...
  public void setMeasuredDimensionImg(int width, int height){
    mViewWidth = width;
    mViewHeight = height;
  }
...
//    mViewHeight = viewHeight - getPaddingTop() - getPaddingBottom();
  ...
  private void drawOverlay(Canvas canvas) {
    ...
    Path path = new Path();
    RectF overlayRect = new RectF((float) Math.floor(mImageRect.left), (float) Math.floor(mImageRect.top),
  ...
  public void startLoad(final Bitmap sourceUri, final LoadCallback callback) {
    loadAsync(sourceUri, callback);
  }

  public void loadAsync(final Bitmap sourceUri, final LoadCallback callback) {
    loadAsync(sourceUri, false, null, callback);
  }

  public void loadAsync(final Bitmap sourceUri, final boolean useThumbnail, final RectF initialFrameRect, final LoadCallback callback) {
  ...
  public Completable loadAsCompletable(final Bitmap sourceUri) {
    return loadAsCompletable(sourceUri, false, null);
  }


  public Completable loadAsCompletable(final Bitmap sourceUri, final boolean useThumbnail,
  ...
  private void applyThumbnail(Bitmap sourceUri) {
    final Bitmap thumb = sourceUri;
    ...
    mExecutor.submit(new Runnable() {
      @Override public void run() {
        ...
        try {
          ...
          mHandler.post(new Runnable() {
            @Override public void run() {
              if (cropCallback != null) cropCallback.onSuccess(cropped, cropRectErase);
  ...
  public void cropAsync(final Bitmap sourceUri, final CropCallback cropCallback, final boolean is_eraser) {
    mExecutor.submit(new Runnable() {
      @Override public void run() {
        try {
          mIsCropping.set(true);
  ...
  public void cropAsync(final CropCallback cropCallback, final boolean is_eraser) {
    cropAsync(null, cropCallback, is_eraser);
  }

  public CropRequest crop(Bitmap sourceUri) {
